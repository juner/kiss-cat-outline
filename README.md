...
本来、`ますきゃっと` のインターフェス仕様は `IMR Industry`(以下 `IMR`) により決められており、 `KISS` もその頃に取り込まれた仕様であるが、 `IMR`解体後の世界では 11人の`ますきゃっと`により運営される団体である `mass cat consortium` により策定されており、 `IMR` 解体後は規則的に5年おきに改定版の仕様を作成している。
